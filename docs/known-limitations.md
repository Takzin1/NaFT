...
- **変更影響解析:** Activity粒度の保守的dependency pathを扱う。適用rule・使用parameterに基づく選択的再検証は実装しているが、任意の実制度Methodologyに対する最小影響集合の完全性は未証明。100 Claimの30 / 70はfixture構成（standard 70 / intensive 15 / intensive_complete 15）に追従する合成実験結果である。
