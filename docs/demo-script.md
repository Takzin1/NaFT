...
ブラウザで `naft-app.html#/reviewer-demo` または `index.html#/reviewer-demo` を開く。ビルド不要。

## 60秒 Reviewer Demo

1. **「100 Claim変更影響実験を実行」**を押す。
2. 方法論 v1 → v2 の変更に対し、ブラウザ上で同じ `analyzeImpact` を再計算する。
3. 画面で以下を確認する。
   - 変更候補：100
   - 再検証対象：30
   - 影響なし：70
...
通常UIのhandler testsはDOM stubを含む。一方、Reviewer DemoについてはGitHub Actions上のheadless Chromeでbutton click後のmetric描画まで実ブラウザsmokeを行う。File chooser / download / accessibility / persistence / responsive visual inspection / 通常6 workflowの網羅QAは未実施。
