...
This branch is Version-aware MRV Evidence Infrastructure. Scope: methodology registry/rule packs, evidence adapters/manifests, field/activity identity, deterministic evaluation/calculation, exception handling, final human attestation, provenance, monitoring packages and program aggregation.
...
- AG-005 is an unconfirmed public-comment reference. Keep CONFIG REQUIRED and calculation.result=null. Prototype Rule Pack release approval cannot change official adoption status.
...
- `tests/smoke.test.js`: dependency-free Node assertions; DOM stubs are not actual browser tests.
