...
**市民参加型GXプロトコルのPoC** — 地域の気候行動について、証憑取込、構造化・不足検知、決定論的チェック、人間審査、ハッシュ連結された検証済み候補記録までを一本化します。非換金ポイント／チケット還元、取引台帳、監査ログ、地域ダッシュボードも同じPoCに統合しています。
...
| 証憑の不足検知・決定論的チェック・ハッシュ連結された候補記録 | 実ファイル保管・正式な第三者検証 |
...
```bash
bash tests/run.sh   # 構文チェック + スモークテスト63項目
```
...
├── tests/                   # スモークテスト63項目（Node標準のみで実行可）
...
| [docs/data-model.md](docs/data-model.md) | 14コレクションのデータモデルと状態遷移 |
...
| [docs/test-report.md](docs/test-report.md) | テスト方針と63項目の結果 |
