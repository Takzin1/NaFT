...
(async function(){
  ...
  assert(pgIEEEDemo().indexOf('Five stages, with a hard human gate') >= 0, 'IEEE demo landing renders');
  ...
  assert(renderModal().indexOf('AIによる参考情報') >= 0, 'review modal shows assist disclosure');
  ...
  assert(environmentalRecordFor('pj9') && /^NAFT-ER-[0-9A-F]{16}$/.test(environmentalRecordFor('pj9').record_hash), 'human approval creates hashed candidate record');
